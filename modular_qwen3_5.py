...
class PoseFeatureProjector(nn.Module):
    """
    Projects flattened MediaPipe pose/face/hand features into the LLM hidden dimension.

    A two-layer MLP with GELU activation maps the concatenated landmark features
    (pose 99-D + face 1434-D + left_hand 63-D + right_hand 63-D = 1659-D by default)
    to the model's hidden size so they can be appended as additional tokens.
    """
    ...
    def forward(self, features: torch.Tensor) -> torch.Tensor:
        """
        Args:
            features: (batch, num_frames, feature_dim) or (num_frames, feature_dim)
        Returns:
            embeddings of same leading shape but last dim = hidden_size
        """
...
class Qwen3_5ForConditionalGeneration(Qwen3_5PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        # --- Pose feature extension ---
        self.use_pose_features = getattr(config, "use_pose_features", False)
        if self.use_pose_features:
            pose_feature_dim = getattr(config, "pose_feature_dim", 1659)
            hidden_size = config.text_config.hidden_size
            self.pose_projector = PoseFeatureProjector(pose_feature_dim, hidden_size)
            print(
                f"[Qwen3.5-Pose] PoseFeatureProjector created: {pose_feature_dim} -> {hidden_size}",
                flush=True,
            )
            # Phase 1: freeze the entire Qwen backbone; only the projection head trains.
    ...
    def _freeze_backbone(self) -> None:
        """Freeze all parameters except pose_projector (Phase-1 training)."""
        ...
        print(
            f"[Qwen3.5-Pose] Backbone frozen. Trainable params: {trainable:,} / {total:,}",
            flush=True,
        )
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        video_grid_thw (`torch.LongTensor` of shape `(num_videos, 3)`, *optional*):
            The temporal, height and width of feature shape of each video in LLM.
        video_paths (`list[str]`, *optional*):
            One video file path per batch item.  When provided and ``config.use_pose_features=True``
            MediaPipe landmarks are extracted on-the-fly and appended as additional tokens after
            the LLM hidden states (VideoNSA style).
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        if not is_first_iteration and use_cache:
            ...
            # Pose features are only needed on the first (prefill) step; the
            # projected pose tokens are appended to hidden_states and captured
            # in the logits for the first generated token.
            model_inputs["video_paths"] = None
        else:
            model_inputs["video_paths"] = video_paths

        return model_inputs

    def _extract_and_project_pose(
        self,
        video_paths: list[str],
        hidden_states: torch.Tensor,
    ) -> Optional[torch.Tensor]:
        """
        Extract MediaPipe landmarks for each video ad-hoc and project them to the
        LLM hidden dimension.

        Extraction happens on CPU (MediaPipe requirement) and is not differentiable;
        gradients flow only through ``self.pose_projector``.

        Args:
            video_paths: One video file path per batch item.  A ``None`` entry
                produces a zero-filled embedding for that sample.
            hidden_states: Current LLM output; used only to determine device/dtype.

        Returns:
            Tensor of shape ``(batch, pose_sample_n, hidden_size)``, or ``None``
            if ``video_paths`` is empty.
        """
        if not video_paths:
            return None

        # Lazy imports — keep heavy dependencies out of the module-level namespace
        # so the model can be imported without MediaPipe / OpenCV installed.
        ...
        try:
            ...
            _logging.getLogger(__name__).error(
                "Pose extraction requires opencv-python and mediapipe: %s", exc
            )
            return None

        sample_n = getattr(self.config, "pose_sample_n", 16)
        feat_dim = getattr(self.config, "pose_feature_dim", 1659)
            ...
            if video_path is not None:
                try:
                    ...
                    features = extract_features_for_frames(
                        str(video_path), frame_indices, feature_types=feature_types
                    )
                    flat = flatten_features(features, feature_types)  # (sample_n, feat_dim)
                except Exception as exc:
                    _logging.getLogger(__name__).warning(
                        "Pose extraction failed for %s: %s — using zeros.", video_path, exc
                    )
            if flat is None:
                flat = torch.zeros(sample_n, feat_dim)
            batch_tensors.append(flat)

        # Stack to (batch, sample_n, feat_dim) and move to projector device/dtype
        pose_features = torch.stack(batch_tensors, dim=0)
        proj_weight = self.pose_projector.proj[0].weight
        pose_features = pose_features.to(device=proj_weight.device, dtype=proj_weight.dtype)

        if not hasattr(self, "_pose_logged"):
            self._pose_logged = True
            print(
                f"[Qwen3.5-Pose] Extracted pose features: {pose_features.shape} "
                f"-> projecting to ({pose_features.shape[0]}, {pose_features.shape[1]}, {proj_weight.shape[0]})",
                flush=True,
            )

        return self.pose_projector(pose_features)  # (batch, sample_n, hidden_size)
