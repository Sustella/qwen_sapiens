...
from multi_dataset_mediapipe import get_all_samples
...
def train(args):
    ...
    with torch.no_grad():
        try:
            _dummy = torch.zeros(1, 4, dtype=torch.long, device=device)
            model(input_ids=_dummy, attention_mask=torch.ones_like(_dummy))
        except Exception as e:
            log.warning("FSDP warm-up forward failed (non-fatal): %s", e)
